"""
Script to download Flickr images.
Author: Ilan Havinga.
Date: February 2023.
"""
...
def open_image(entry):
    ...
    if image_url != image_url:
        ...
        print("low resolution image not available..checking for higher resolutions...")
        
        image_url = entry.iloc[2]

        if image_url != image_url:
            image_url = entry.iloc[3]
...
def main():
    ...
    image_urls = pd.read_csv(f"{proj_dir}/data/flickr/urls/flickr_urls_{s}.csv")
    
    image_dir = f"{proj_dir}/data/flickr/imgs" # results file id (later split between atts and scenes)
    ...
    for i in range(start_i, len(image_urls)):
        ...
        if image: # if list is not empty
            ...
            image_urls.to_csv(f"{proj_dir}/data/flickr/urls/flickr_urls_{s}.csv", sep=',', index = False)
