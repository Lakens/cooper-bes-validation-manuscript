...
# column with a genuine boolean Cooper-vs-metacheck comparison in this
# validation project:
...
#
# This compares the three CORE Metacheck modules' recreated columns
# (mc_data_availability, mc_data_archive, mc_data_license, mc_data_download,
# mc_code_archived, mc_code_download, mc_any_readme -- all derived from
# repo_check/data_check/code_check output by
# final_pipeline/02_build_comparison_data.R) against Cooper et al.'s own
# coded ground truth. It is a copy of
# cooper_validation_metacheck/manuscript/code_data_preparation/
# 25_compute_comparison_statistics.R, relocated here with its own inputs
# and output so this stage of the pipeline is self-contained; only the
# file paths were changed (all now relative to this folder instead of
# data/), and this header comment.
#
# Two data sources, used for different things:
#   - 02_create_comparison_data/master_comparison.rds (side_by_side below,
#     1861 rows): the FULL corpus, agreements included -- used for
#     confusion tables, N's, sensitivity/specificity, McNemar's test.
#     master_comparison.rds is read here (as `side_by_side`, its original
#     variable name in the source script this is copied from) rather than
#     a separate cooper_vs_recreated.rds, since master_comparison.rds is
#     a strict superset of that file's columns (it is built FROM
#     cooper_vs_recreated.rds, adding only mc_data_open/mc_code_open/
#     oddpub_*/recorder_ID -- see 02_create_comparison_data/
#     04_build_master_comparison.R) -- every column this script actually
#     uses (the mc_*/Cooper columns below) is identical either way, so
#     reading the superset avoids keeping two separate copies of the same
#     upstream data in sync across folders.
#   - disagreement_review_worklist.csv (disagreements only, each with an
...
#
# Usage (from inside manuscript/): Rscript 03_comparing_results/01_compute_comparison_statistics.R

status <- function(...) cat(sprintf("[%s] %s\n", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), sprintf(...)))
`%||%` <- function(a, b) if (is.null(a)) b else a
library(dplyr)

side_by_side <- readRDS("02_create_comparison_data/master_comparison.rds")  # 1861 rows
...
# any_readme lives only in the worklist (built by
# cooper_validation_metacheck's 09_add_any_readme_column.R from the full
# corpus, but not re-saved as its own full-corpus RData object) --
# reconstruct its full-corpus version the same way that script did,
# directly from side_by_side's data_README/code_README + mc_data_README,
# so the full N (agreements included) is available here too.
...
for (nm in names(bool_pairs)) {
  ...
  # The two asymmetric rates: "Cooper got what metacheck missed X% of the
  # time" -- among all cases where the two disagree (fn + fp), what
  # fraction are cases where Cooper said Yes and metacheck said No
  # (metacheck's misses)?
  n_disagree <- fn + fp
...
save(results, verdict_tallies, file = "03_comparing_results/comparison_statistics.RData")
status("Saved 03_comparing_results/comparison_statistics.RData")
