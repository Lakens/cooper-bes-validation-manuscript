...
# nfields: integer, number of tag fields used (typically 4: species, contact, distance, angle)
...
extract.records <- function(data, nfields, tagcolumn, placecolumn, filecolumn, datecolumn, othercolumns=NULL, date.format="%Y:%m:%d %H:%M:%S"){
  ...
  repcols <- dat[,-pid]
  maxind <- ncol(repcols)/nfields
  colnames(repcols) <- rep(colnames(repcols)[1:nfields], maxind)
  res <- NULL
  for(i in 0:(maxind-1))
    res <- rbind(res, cbind(leadcols, repcols[,(1:nfields)+i*nfields]))
