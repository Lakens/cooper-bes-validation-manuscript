...
nutrient_list <- list(biomass_index_wide = biomass_index_wide, 
                      calcium_wide = calcium_wide, 
                      iron_wide = iron_wide, 
...
species_list <- species_list %>% 
  filter(valid_name_FishBase %in% gsub('_', ' ', keep_species$valid_name_FishBase), 
         valid_name_FishBase %in% gsub('_', ' ', fished_genus))
...
for(i in seq_along(saup_reef_tonnes)){
   ...
   nutrient_output_country <- lapply(nutrient_list, function(x){
     ...
     # obtain the maximum potential of the system when fished at n_fished_species
     ...
     nutrient_nullFished_raw  <- sapply(1:999, function(x) sum(colMeans(nutrient_country[,sample(seq_along(colnames(nutrient_country)), 
                                                           n_fished_species)],na.rm=T), na.rm = T))
...
# nationally categories, even when considered at the genus level which could be a large underestimate of the locally fished species. 
...
png(filename = 'figures-final-v4/05-nutrient-simulation-FAMILY/YIELD-allNutrients_maximum.png', 
    width = 1900, height = 1900, res = 300)
YIELD_plot <- nutrient_sim_all_plot %>% dplyr::select(Country_GADM, max_nutrient, nutrient_trueFished_raw, nutrient) %>% unique() %>% na.omit() %>% data.frame()
YIELD_plot <- YIELD_plot %>% filter(nutrient != 'biomass_index_wide')
ggplot() + 
  geom_col(data = YIELD_plot, 
  ...
  facet_wrap(~nutrient, nrow = 4, scales = 'free_y') + 
  theme_bw() + 
  ...
  scale_colour_manual(breaks = c('biomass_index_wide', 'calcium_wide', 'iron_wide', 'vitamina_wide', 'zinc_wide'), 
                      values = c('gray50', '#00D1CD', '#ff8a00', '#00ad0b', '#FF76C5')) + 
  scale_fill_manual(breaks = c('biomass_index_wide', 'calcium_wide', 'iron_wide', 'vitamina_wide', 'zinc_wide'), 
                      values = c('gray50', '#00D1CD', '#ff8a00', '#00ad0b', '#FF76C5')) + 
  xlab(NULL) + 
  ylab('nutrient yield (g)')
dev.off()
...
# country point plots
png(filename = 'figures-final-v4/05-nutrient-simulation-FAMILY/YIELD-nutrient-selectivity.png', 
...
nutrient_sim_all_plot %>% 
  ...
  na.omit() %>% 
  t() %>% 
  write.csv(., file = 'figures-final-v4/05-nutrient-simulation-FAMILY/YIELD-nutrient-optimality-statistics.csv')
...
nutrient_statistics_summary <- nutrient_sim_all_plot %>% 
  ...
  do(mean_estimate  = mean(.$estimate, na.rm = T),
     ...
     significant_0.001 = sum(.$p.value < 0.001) / length(.$esstimate), 
...
write.csv(nutrient_statistics_summary %>% 
            dplyr::select(nutrient, mean_estimate, sd_estimate, 
                     ...
                     significant_0.001, n_countries) %>% 
            t(), 
          file = 'figures-final-v4/05-nutrient-simulation-FAMILY/YIELD-selectivity-statistics.csv')



# save in clean format summary table
writexl::write_xlsx(
  nutrient_sim_all_plot %>% 
    na.omit() %>% 
    ...
    mutate(Micronutrient = recode(.$Micronutrient, 
                                  ...
                                  'zinc_wide' = 'Zinc')), 
  path = 'figures-final-v4/05-nutrient-simulation-FAMILY/YIELD-selectivity-summary.xlsx')
...
nutrient_sim_all_plot %>% 
  ...
  mutate(Micronutrient = recode(.$Micronutrient, 
                                ...
                                'zinc_wide' = 'Zinc (mg)')) %>% 
  writexl::write_xlsx(., path = 'figures-final-v4/05-nutrient-simulation-FAMILY/YIELD-all-metric-summary.xlsx')
