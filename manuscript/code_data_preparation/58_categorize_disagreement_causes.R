...
#
# Usage: Rscript 58_categorize_disagreement_causes.R
...
# code_archived-only bucket, added during the final (post-A.5) review
# round: repo_check DID find the paper's own GitHub/CRAN R-package
# repository, but the R-package-source-tree exclusion (one of A.5's own
# fixes) stripped the package's real R/ source directory from the
# listing along with the bundled-dependency scaffolding it was meant to
# remove -- a distinct cause from either "repository never found" or
# "code bundled but not classified" above, and confirmed as a genuine
# regression rather than a pre-existing gap (see the Discussion section).
...
save(cooper_right_causes, any_readme_dd_causes, data_availability_dd_causes, code_archived_mr_causes,
    file = "data/disagreement_cause_categories.RData")
status("Saved data/disagreement_cause_categories.RData")
